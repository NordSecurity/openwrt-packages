...
teliod_bin, version = sys.argv[1], sys.argv[2]

with open(teliod_bin, "rb") as f:
    data = f.read()
...
start, end = m.span()
field_len = end - start  # includes final NUL
max_len = field_len - 1  # subtract the final NUL

print(f"Replacing version placeholder in {teliod_bin} with '{version}'")
print(f"Field length: {field_len} bytes, max version length: {max_len} bytes")
